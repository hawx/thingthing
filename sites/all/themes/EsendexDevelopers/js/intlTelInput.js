...
Plugin.prototype = {
    ...
    _selectListItem: function(listItem) {
        ...
        // focus the input
        this.telInput.focus();
        $('#form-inputs .userPhoneNumber').focus();
        changeLeftPos(newNumber);
    },
...
if($("#FT-PhoneNumber").length){
	...
	switch (getCookie("territory")) {
		case "ES":
			$("#FT-PhoneNumber").intlTelInput({defaultCountry: 'es', preferredCountries: ['es']});
		break;
		case "FR":
			$("#FT-PhoneNumber").intlTelInput({defaultCountry: 'fr', preferredCountries: ['fr', 'be', 'ch', 'lu']});
		break;
		case "IE":
			$("#FT-PhoneNumber").intlTelInput({defaultCountry: 'ie', preferredCountries: ['ie', 'gb', 'us']});
		break;
		case "AU":
			$("#FT-PhoneNumber").intlTelInput({defaultCountry: 'au', preferredCountries: ['au', 'nz']});
		break;
		case "US":
			$("#FT-PhoneNumber").intlTelInput({defaultCountry: 'us', preferredCountries: ['us', 'gb']});
		break;
		case "DE":
			$("#FT-PhoneNumber").intlTelInput({defaultCountry: 'de', preferredCountries: ['de', 'at', 'ch']});
		break;
		default:
			$("#FT-PhoneNumber").intlTelInput({defaultCountry: 'gb', preferredCountries: ['gb', 'ie', 'us']});
	}
		if(typeof(prefix) != "undefined" && prefix !== null) {
			$("#FT-PhoneNumber").intlTelInput("setNumber", prefix);
			changeLeftPos(prefix);
		}

	//Focus the editable field when click on the prefix
	$("#FT-PhoneNumber").click(function() {
		$('#form-inputs .userPhoneNumber').focus();
	});
	
	//Change background position as other fields do
	var origBackgroundPos = $("#FT-PhoneNumber").css('background-position');
	$('#form-inputs .userPhoneNumber').focusin(function() {
		$("#FT-PhoneNumber").css('background-position','center bottom');
	});
	$('#formdesign14 #form-inputs .userPhoneNumber').focusin(function() {
		$("#FT-PhoneNumber").css('background-position','-33px -363px');
	});
	$('#form-inputs .userPhoneNumber').focusout(function() {
	//	$("#FT-PhoneNumber").css('background-position', origBackgroundPos)
		$("#FT-PhoneNumber").attr('style', '');
	});	
	
	//change the colour of the background
	var origBackgroundCol = $("#FT-PhoneNumber").css('background-color');
	$('#SignUp #form #form-inputs .userPhoneNumber').focusin(function() {
		$("#FT-PhoneNumber").css('background','white');
	});
	$('#SignUp #form #form-inputs .userPhoneNumber').focusout(function() {
		$("#FT-PhoneNumber").css('background',origBackgroundCol);
	});		
}

function changeLeftPos(countryCode) {
	countryCode=$.trim(countryCode);
	if(countryCode.length>3) {
		$('#form-inputs .userPhoneNumber').css('left',''); 
		//Reset left property to add 20px over the original positioning
		var newLeft = parseInt($('#form-inputs .userPhoneNumber').css('left'));
		newLeft+=14;
		$('#form-inputs .userPhoneNumber').css('left',newLeft+'px');
	} else {
		$('#form-inputs .userPhoneNumber').css('left',''); 
	}
}
